...
callback      | Function that is called at every iteration with the current primal solution and the current number of iterations, the algorithm stops if true is returned
